...
const CategoryItem = ({ item, index, categoriesMenuData }) => {
  ...
  return (
    item?.href ?
      ...
      <div
        className={`py-3 px-4 cursor-pointer flex justify-between items-center hover:text-amber-700 ${index !== categoriesMenuData.length - 1 && 'border-b border-0.5 border-gray-700'}`}
        key={item.id}
        onMouseEnter={() => setOpenSubMenu(true)}
        onMouseLeave={() => setOpenSubMenu(false)}
      >
